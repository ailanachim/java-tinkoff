...
public class Solution {
    ...
    public static Animal maxByNameLength(List<Animal> animals) {
        return animals.stream().max(Comparator.comparing(animal -> animal.name().length())).orElseThrow();
    }
    ...
    public static Animal kThMaxByAge(List<Animal> animals, int k) {
        return animals.stream().sorted(Comparator.comparing(Animal::age).reversed()).limit(k)
            .min(Comparator.comparing(Animal::age)).orElseThrow();
    }
    ...
    public static Integer sumOfPaws(List<Animal> animals) {
        return (int) animals.stream().collect(Collectors.summarizingInt(Animal::paws)).getSum();
    }
    ...
    public static Boolean spidersBiteMoreOftenThanDogs(List<Animal> animals) {
        List<Animal> spidersAndDogs =
            animals.stream().filter(animal -> animal.type() == Animal.Type.SPIDER || animal.type() == Animal.Type.DOG)
                .toList();

        Map<Animal.Type, Long> animalCount =
            spidersAndDogs.stream().collect(Collectors.groupingBy(Animal::type, Collectors.counting()));

        Map<Animal.Type, Long> biteAnimalCount =
            spidersAndDogs.stream().filter(Animal::bites)
                .collect(Collectors.groupingBy(Animal::type, Collectors.counting()));

        double spiderProportion =
            (double) biteAnimalCount.getOrDefault(Animal.Type.SPIDER, 0L)
                / animalCount.getOrDefault(Animal.Type.SPIDER, 1L);

        double dogProportion =
            (double) biteAnimalCount.getOrDefault(Animal.Type.DOG, 0L)
                / animalCount.getOrDefault(Animal.Type.DOG, 1L);
    ...
    public static Animal fishWithMaxWeight(List<Animal> list1, List<Animal> list2, List<Animal>... animals) {
        ...
        return stream.filter(animal -> animal.type() == Animal.Type.FISH).max(Comparator.comparing(Animal::weight))
            .orElseThrow();
    }

    // task 19
    public static Map<String, Set<ValidationError>> getValidationErrors(List<Animal> animals) {
        return animals.stream().collect(Collectors.toMap(Animal::name, Solution::validateAnimal));
    }
    ...
    public static Map<String, String> transformErrorsToString(Map<String, Set<ValidationError>> errorMap) {
        return errorMap.entrySet().stream().map(entry -> Map.entry(entry.getKey(), entry.getValue().stream().map(
            ValidationError::message).reduce(((s, s2) -> s + ", " + s2)).orElse(""))).collect(Collectors.toMap(
            Map.Entry::getKey,
            Map.Entry::getValue
        ));
    }
